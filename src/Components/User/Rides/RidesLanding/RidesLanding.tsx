...
const updateRides = async (url: string, date: string, emailMsg: string) => {
  ...
  try {
    // delete Old Rides
    const oldRides = await (API.graphql(graphqlOperation(listRides)) as Promise<any>).then((result) => {
    ...
    for (const oldRide of oldRides) {
      await (API.graphql(graphqlOperation(deleteRide,
        { input: { id: oldRide.id } })) as Promise<any>).then((result) => {
          console.log(result);
        }).catch(reason => {
    ...
    for (const row of rows) {
      if (row.get('driver_name') && row.get('rider_name')) {
        ...
        } else { // car doesn't exist, create car
          carInputs.push({
    ...
    await (API.graphql(graphqlOperation(createRide, {
      input: rides
...
export const RidesLanding = () => {
  ...
  useEffect(() => {
    const fetchRides = async () => {
      await (API.graphql(graphqlOperation(listRides)) as Promise<any>).then((result) => {
...
const RidesSettings = () => {
  ...
  return <div className="admin">
    ...
    <form className={'admin-form'} onSubmit={async (e) => {
      ...
      <label>Rides Date</label><br />
      <input type="date" required className={'rides-input'} value={date || ''} onChange={(e) => setDate(e.target.value)} /><br />
      <label>Custom Email Message</label><br />
      <input type="text" required className={'rides-input'} value={emailMsg} onChange={(e) => setEmailMsg(e.target.value)} /><br />
      <Button className={'upload-rides-button'} type="submit">Upload Rides</Button>
    </form>
  </div>
